...
#| message: false
#| echo: false
#| tbl-cap: "Gj.snitt og st.avvik for watt-maks"
#| label: tbl-tidwatt
...
Den gjennomsnittlige watt-verdien som testdeltakerne holdt de siste 30 sekundene av $\dot{V}O_{2max}$-testene var `r mean_and_sd` og den typiske feilen ble målt til `r te`. Variasjonskoeffisienten ble beregnet til `r cv_perc` % (se @fig-sumstat). 


## Diskusjon

Den gjennomsnittlige watt-verdien som testdeltakerne holdt de siste 30 sekundene av $\dot{V}O_{2max}$-testen var `r mean_and_sd` og den typiske feilen ble målt til `r te`. Ifølge Hopkins (2000) er den typisk feilen, den gjennomsnittlige variasjonen ved en test. Den typiske feilen gir en indikasjon på den forventede variasjonen ved en retest [@Hopkins]. I dette tilfellet kan en forvente en variasjon på `r cv_perc` % ved en eventuell re-test.

Presentere cv. Styrker (utstyr, lærer med mye praktisk erfaring) og svakheter (ulike testledere, protokoll ukjent) 
